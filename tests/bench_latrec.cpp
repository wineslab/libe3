 ...
 *
 * Reports the cost of a stamp into an open ring and into a thread that has no
 * ring, and fails if either exceeds a ceiling. The ceilings are wide enough to
...
double bench(const char* label, latrec_t* ring, int iters) {
    ...
    for (int i = 0; i < 1000; i++) latrec_stamp(ring, 1, LATREC_ENQUEUE, 0, 0);
    const auto t0 = std::chrono::steady_clock::now();
    for (int i = 0; i < iters; i++) {
        latrec_stamp(ring, static_cast<uint64_t>(i), LATREC_ENQUEUE,
...
int main() {
    ...
    // A ring that was never opened: what a stamp costs on a thread that has
    // no ring of its own. Threads in that state share one such ring, so this
    // is the floor the enabled cost is measured against.
    latrec_t off;
    memset(&off, 0, sizeof(off));
    const double disabled_ns = bench("no ring on this thread", &off, kIters);
    ...
    if (!dir) {
        std::fprintf(stderr, "ERROR: could not create a temp ring directory\n");
        return 1;
    }
    latrec_t on;
    if (latrec_open_in(&on, dir, "bench.enabled", 22) != 1) {
        std::fprintf(stderr, "ERROR: latrec_open_in failed under %s\n", dir);
        return 1;
    }
    ...
    if (disabled_ns > kMaxDisabledNs) {
        std::fprintf(stderr, "ERROR: disabled stamp %.2f ns exceeds %.2f ns ceiling"
                             " -- the no-ring check is no longer free\n",
