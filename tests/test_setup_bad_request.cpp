 ...
 *
 *   (1) Sending undecodable bytes on the setup channel produces a reply
 *       (best-effort empty), i.e. the REQ/REP exchange always completes.
 *
...
TEST(SetupChannel_garbageRequest_repliesAndChannelSurvives) {
    ...
    // (1) Garbage on the setup channel: a reply must still arrive (the
    //     agent sends a best-effort empty frame). Without the fix the agent
