...
size_t drops_with_reason(uint64_t reason) {
    ...
    for (const auto& r : g_recs)
        if (stage_of(r) == LATREC_L9_DROP && r.aux2 == reason) n++;
    return n;
}
...
TEST(a_queue_push_drop_ends_its_outbound_message) {
    ...
    for (const auto& r : g_recs) {
        ...
        if (stage_of(r) == LATREC_L9_DROP) reason_by_seq[seq_of(r)] = r.aux2;
    }
    size_t checked = 0;
    for (const auto& [seq, stages] : stages_by_seq) {
        ...
        ASSERT_EQ(stages.count(LATREC_L1_DEQUEUE), 0u);
        ASSERT_EQ(stages.count(LATREC_L2_ENCODE_DONE), 0u);
        ASSERT_EQ(stages.count(LATREC_L3_SEND_DONE), 0u);
        checked++;
    }
...
TEST(every_drop_carries_a_known_reason) {
    ...
    for (const auto& r : g_recs) {
        if (stage_of(r) != LATREC_L9_DROP) continue;
        total++;
        seen[r.aux2]++;
