 ...
 *
 *   (2) Surfaces overflow as an explicit push() != SUCCESS return value
 ...
 *
 * The tests use LockFreeQueue<DAppReport>(1024) — the same wrapper type
...
TEST(ReportQueue_drainsAllMessagesUnderCapacity) {
    LockFreeQueue<DAppReport> q(REPORT_QUEUE_CAPACITY);
    constexpr uint32_t N = 500;

    for (uint32_t i = 0; i < N; ++i) {
        ASSERT_TRUE(q.push(make_report(i)) == ErrorCode::SUCCESS);
    }
    ...
    ASSERT_FALSE(q.try_pop().has_value());    // queue drained
}
...
TEST(ReportQueue_overflowReturnsFalseInsteadOfSilentDrop) {
    ...
    for (size_t i = 0; i < REPORT_QUEUE_CAPACITY * 2; ++i) {
        if (q.push(make_report(static_cast<uint32_t>(i))) == ErrorCode::SUCCESS) {
            ++pushed;
        }
    }

    // The queue rounds requested capacity up to the next power of two,
    // so `pushed` may be ≥ REPORT_QUEUE_CAPACITY.  The invariant we
    // assert: at least one push attempt failed (returned non-SUCCESS),
    // proving that overflow surfaces explicitly.
    ASSERT_GE(pushed, REPORT_QUEUE_CAPACITY);
    ASSERT_LT(pushed, REPORT_QUEUE_CAPACITY * 2);

    size_t popped = 0;
    while (q.try_pop().has_value()) ++popped;
...
TEST(ReportQueue_burstProducerSlowConsumer_noLoss) {
    ...
    for (uint32_t i = 0; i < N; ++i) {
        while (q.push(make_report(i)) != ErrorCode::SUCCESS) {
    ...
    for (uint32_t p = 0; p < N_PRODUCERS; ++p) {
        producers.emplace_back([&q, p]() {
            ...
            for (uint32_t i = 0; i < N_PER_PRODUCER; ++i) {
                while (q.push(make_report(base + i)) != ErrorCode::SUCCESS) {
