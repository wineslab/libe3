...
The full API reference is automatically generated with [Doxygen](https://www.doxygen.nl/) and published to GitHub Pages on every release. See [docs/latrec.md](docs/latrec.md) for the latency-recording (`latrec`) guide: the mechanism, the clock model, ring naming/sizing, the stage catalog, and the capture-to-CSV workflow.
