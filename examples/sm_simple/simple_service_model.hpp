 ...
 *
 * This SM is also the reference implementation of the source-side stage
 * catalog: it stamps the same RECORD / ENCODE_E3SM / DECODE_E3SM identifiers
 * (include/libe3/latrec.h) that any other data source and RAN-side Service
 * Model stamps, keyed on its own indication counter, rather than a catalog
 * block of its own. Every stamp resolves to a no-op unless the library was
...
public:
    ...
    libe3::ErrorCode handle_control_action(
        ...
        latrec_tstamp(seq, LATREC_DECODE_E3SM_BEGIN, action.action_data.size(),
                      static_cast<uint64_t>(libe3::PduType::DAPP_CONTROL_ACTION));

        int sampling = 0;
        bool decode_ok = libe3_examples::decode_simple_control(action.action_data, sampling, encoding_);

        latrec_tstamp(seq, LATREC_DECODE_E3SM_DONE, decode_ok ? 1 : 0,
                      static_cast<uint64_t>(libe3::PduType::DAPP_CONTROL_ACTION));
...
private:
    ...
    void worker_loop() {
        ...
        // nothing else opens its latrec ring; the source-side stamps below
        // (and the latrec_ctx_set() bridge) are no-ops until this runs. The
        // role names the ring, which is how a reader tells this SM's
        // RECORD/ENCODE_E3SM records from any other component's.
        latrec_tls_open_as("sm_simple");
        ...
        while (running_) {
            ...
            latrec_tstamp(seq, LATREC_RECORD_BEGIN, 0, 0);
            ...
            latrec_tstamp(seq, LATREC_ENCODE_E3SM_DONE, encoded.size(),
                          static_cast<uint64_t>(libe3::PduType::INDICATION_MESSAGE));
            // Bridges this SM's own seq to libe3's Pdu::enqueue_seq:
            // EMIT_ENTER's aux captures whatever latrec_ctx_set() last carried,
            // which is what joins the A1..A3 records above to the A4..A6
            // outbound leg below (see LATREC_EMIT_ENTER's doc comment).
            latrec_ctx_set(seq);
